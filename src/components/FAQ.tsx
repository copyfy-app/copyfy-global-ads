...
const FAQ = () => {
  ...
  return <>
      ...
      <section className="mb-12 sm:mb-20 px-4">
        ...
        <div className="max-w-4xl mx-auto">
          <Accordion type="single" collapsible className="space-y-3 sm:space-y-4">
            {faqs.map((faq, index) => <AccordionItem key={index} value={`item-${index}`} className="bg-black/60 backdrop-blur-sm rounded-xl sm:rounded-2xl border border-yellow-500/20 px-4 sm:px-6 hover:border-yellow-500/40 transition-all duration-300">
      ...
      <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50">
        <Button onClick={() => setIsFloating(!isFloating)} className="bg-yellow-500 hover:bg-yellow-600 text-black rounded-full w-12 h-12 sm:w-14 sm:h-14 shadow-2xl shadow-yellow-500/50 transition-all duration-300 hover:scale-110">
          {isFloating ? <X className="h-5 w-5 sm:h-6 sm:w-6" /> : <HelpCircle className="h-5 w-5 sm:h-6 sm:w-6" />}
        </Button>
      </div>

      {/* Floating FAQ Panel */}
      {isFloating && <div className="fixed bottom-16 right-4 sm:bottom-24 sm:right-6 z-40 w-80 sm:w-96 max-h-80 sm:max-h-96 bg-black/95 backdrop-blur-lg rounded-xl sm:rounded-2xl border border-yellow-500/30 shadow-2xl overflow-hidden">
          ...
          <div className="p-3 sm:p-4 max-h-64 sm:max-h-80 overflow-y-auto">
            <Accordion type="single" collapsible>
              {faqs.slice(0, 3).map((faq, index) => <AccordionItem key={index} value={`floating-${index}`} className="border-yellow-500/20">
            ...
            <Button onClick={() => setIsFloating(false)} variant="outline" className="w-full mt-3 sm:mt-4 border-yellow-500/50 text-yellow-500 hover:bg-yellow-500 hover:text-black text-xs sm:text-sm">
